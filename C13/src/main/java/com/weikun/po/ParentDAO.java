 ...
 * Parent entities. Transaction control of the save(), update() and delete()
 * operations can directly support Spring container-managed transactions or they
 * can be augmented to handle user-managed Spring transactions. Each of these
 * methods provides additional information for how to configure it for the
 * desired type of transaction control.
 * 
 * @see com.weikun.po.Parent
...
public class ParentDAO extends BaseHibernateDAO {
	...
	public Parent findById(java.lang.Integer id) {
		...
		try {
			Parent instance = (Parent) getSession().get("com.weikun.po.Parent",
					id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Parent instance) {
		...
		try {
			List results = getSession().createCriteria("com.weikun.po.Parent")
	...
	public List findAll() {
		log.debug("finding all Parent instances");
		try {
	...
	public Parent merge(Parent detachedInstance) {
		...
		try {
			Parent result = (Parent) getSession().merge(detachedInstance);
